...
export const App: React.FC = () => {
  ...
  const fetchInitialData = async () => {
    try {
      const [stylesData, sysData] = await Promise.all([
        api.getStyles(),
        api.getSystemStatus()
      ]);
  ...
  const pollJobStatus = async (jobId: string) => {
    try {
      ...
      // Adaptive polling interval: 250ms when generating, 600ms when preparing/loading
      const delay = (job.state === 'generating') ? 250 : 600;
