...
class JobQueue:
    ...
    def cancel_job(self, job_id: str) -> bool:
        if job_id in self.jobs:
            ...
            if job.state in [JobState.WAITING, JobState.LOADING_MODEL, JobState.GENERATING, JobState.PREPARING]:
    ...
    async def _process_queue(self):
        while True:
            ...
            try:
                ...
                # Check if it was CUDA Out Of Memory
                if "out of memory" in error_msg.lower() or "cuda oom" in error_msg.lower():
                    vram_manager.safe_empty_cache()
                    next_strat = vram_manager.step_down_strategy()
                    if next_strat and settings.vram.oom_retry_enabled:
                        job.error_message = (
                            f"CUDA Out Of Memory encountered on RTX 5080. Automatically switched "
                            f"VRAM strategy to {next_strat.value} and safely reclaimed memory. Please retry."
                        )
                    else:
                        job.error_message = (
                            "CUDA Out of Memory on RTX 5080. Unload other models or reduce image resolution."
                        )
                else:
                    job.error_message = f"Generation failed: {error_msg}"
            finally:
                job.metrics.total_time = round(time.time() - t_start, 2)
                if job.metrics.total_time > 0:
    ...
    async def _execute_job(self, job: GenerationJob):
        # 1. Routing & Prompt Analysis
        job.state = JobState.PREPARING
        t_prep_start = time.time()

        if job.model == "auto":
            decision = auto_router.analyze(job.prompt, user_mode=job.mode)
        ...
        if settings.lm_studio.enabled:
            job.enhanced_prompt = await lm_studio_client.enhance_prompt(job.prompt)
        else:
            from backend.app.prompt_engine.local_analyzer import local_analyzer
            job.enhanced_prompt = local_analyzer.enhance(job.prompt)
        ...
        # 2. Loading Model & VRAM enforcement
        job.state = JobState.LOADING_MODEL
        t_load_start = time.time()
        job.vram_strategy_used = vram_manager.current_strategy.value

        # If requested model has no weights on disk, check if an installed model is available
        installed = [mid for mid, ad in model_registry.adapters.items() if ad.has_weights()]
        req_adapter = model_registry.get_adapter(target_model_id)
        if (not req_adapter or not req_adapter.has_weights()) and installed:
            chosen = "zimage-turbo" if "zimage-turbo" in installed else ("sdxl" if "sdxl" in installed else installed[0])
            app_logger.warning(f"Model '{target_model_id}' has no weights. Falling back to installed '{chosen}'.")
            job.routing_reason = (job.routing_reason or "") + f" [Weights missing for '{target_model_id}', routed to installed '{chosen}']"
            target_model_id = chosen

        adapter = model_registry.load_model(target_model_id, vram_strategy=job.vram_strategy_used)
        job.metrics.model_load_time = round(time.time() - t_load_start, 2)

        # Attach LoRAs if specified
        ...
        prompt_to_use = job.enhanced_prompt if job.enhanced_prompt else job.prompt

        # Run model generation in thread pool to prevent blocking asyncio loop
        ...
        gen_logger.info(
            f"Job {job.id} completed in {job.metrics.total_time}s | Model: {target_model_id} | Size: {job.width}x{job.height}"
        )
